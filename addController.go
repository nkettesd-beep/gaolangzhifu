...
type Urole struct {
	...
	CreateTime     string
	BasicRate      int
	RoadTotalLimit string
	RoadEverydayLimit string
	RoadRemark   string
...
func (c *AddController) AddRoad() {
	...
	roadName := strings.TrimSpace(u.RoadName)
	roadRemark := strings.TrimSpace(u.Remark)
	productUid := strings.TrimSpace(u.ProductName)
	TelegramBank := strings.TrimSpace(u.TelegramBank)
	TelegramMsg := strings.TrimSpace(u.TelegramMsg)
	TelegramAds := strings.TrimSpace(u.TelegramAds)
	WhiteIps := strings.TrimSpace(u.WhiteIps)
	TelegramUid :=strings.TrimSpace(u.TelegramUid)


	payType := strings.TrimSpace(u.PayType)
	payTypelei := strings.TrimSpace(u.PayTypelei)
	payleiname :=road.IsExistByUidAndPayType(payTypelei)
	basicRate := strconv.FormatFloat(u.BasicFee, 'f', -1, 64)
	settleFee := strconv.FormatFloat(u.SettleFee, 'f', -1, 64)
	roadTotalLimit :=strconv.FormatFloat(u.TotalLimit, 'f', -1, 64)
	roadEverydayLimit :=strconv.FormatFloat(u.TodayLimit, 'f', -1, 64)
	singleMinLimit := strconv.FormatFloat(u.SingleMinLimit, 'f', -1, 64)
	singleMaxLimit := strconv.FormatFloat(u.SingleMaxLimit, 'f', -1, 64)
	startHour := "0"
	endHour := u.EndHour
	params := strings.TrimSpace(u.Params)
	se := new(service.AddService)
	dataJSON := se.AddRoad(startHour, endHour, roadName, productUid, payType,payleiname,payTypelei, basicRate, settleFee,
		roadTotalLimit, roadEverydayLimit, singleMinLimit, singleMaxLimit, roadUid, roadRemark,TelegramBank,TelegramMsg,TelegramAds,WhiteIps,TelegramUid, params)

	c.GenerateJSON(dataJSON)
}


func (c *AddController) AddRoadxun() {

	id := strings.TrimSpace(c.GetString("Id"))
	roadUid := strings.TrimSpace(c.GetString("RoadUid"))

	se := new(service.AddService)
	dataJSON:= se.AddRoadxuan(id,roadUid)
...
* 添加通道
 */
func (c *AddController) AddRoadedit() {
	var u Urole
	json.Unmarshal(c.Ctx.Input.RequestBody, &u)
	...
	params := strings.TrimSpace(u.Params)
	se := new(service.AddService)
	dataJSON := se.AddRoad(startHour, endHour, roadName, productUid, payType,payTypelei,payleiname, basicRate, settleFee,
		roadTotalLimit, roadEverydayLimit, singleMinLimit, singleMaxLimit, roadUid, roadRemark,TelegramBank,TelegramMsg,TelegramAds,WhiteIps,TelegramUid, params)
...
* 添加或者更新通道池中的通道
 */
func (c *AddController) SaveRoadUidapi() {
	roadel:=strings.TrimSpace(c.GetString("el"))
	roadUids := strings.TrimSpace(c.GetString("roaduid"))
	roadPoolCode := strings.TrimSpace(c.GetString("roadPoolCode"))

	se := new(service.AddService)

	dataJSON := se.SaveRoadUidtongdao(roadPoolCode,roadel, roadUids)
...
func (c *AddController) AddAgenttelegram() {
	agentName := strings.TrimSpace(c.GetString("agentName"))
	agentPhone := strings.TrimSpace(c.GetString("agentPhone"))
	agentLoginPassword := strings.TrimSpace(c.GetString("agentLoginPassword"))
	agentVertifyPassword := strings.TrimSpace(c.GetString("agentVertifyPassword"))
...
func (c *AddController) AddAgenttelegramurl() {
	agentName := strings.TrimSpace(c.GetString("agentName"))
	agentPhone := strings.TrimSpace(c.GetString("agentPhone"))
	agentLoginPassword := strings.TrimSpace(c.GetString("agentLoginPassword"))
	address := strings.TrimSpace(c.GetString("agentName"))
	rakmart := strings.TrimSpace(c.GetString("rakmart"))

	se := new(service.AddService)

	keyDataJSON := se.Addtelegraminfo(agentName, agentPhone, agentLoginPassword,address,rakmart)

	c.GenerateJSON(keyDataJSON)
}


func (c *AddController) AddAgenttelegramubuid() {
	marchuid:=strings.TrimSpace(c.GetString("marchuid"))
	getpageone:=strings.TrimSpace(c.GetString("getpageone"))
	getpagetow:=strings.TrimSpace(c.GetString("getpagetow"))
	...
	keyDataJSON := se.Telegramonbyidurl(getpageone,getpagetow,marchuid)
...
* 添加运营
 */
func (c *AddController) AddMauser() {
	agentName := strings.TrimSpace(c.GetString("merchantName"))
	agentPhone := strings.TrimSpace(c.GetString("phone"))
	agentLoginPassword := strings.TrimSpace(c.GetString("loginPassword"))
	agentVertifyPassword := strings.TrimSpace(c.GetString("verifyPassword"))
	agentRemark := strings.TrimSpace(c.GetString("remark"))
	status := strings.TrimSpace(c.GetString("merchantStatus"))
	agentUid := strings.TrimSpace(c.GetString("agentUid"))

	se := new(service.AddService)
	keyDataJSON := se.AddmauserAgent(agentName, agentPhone, agentLoginPassword, agentVertifyPassword, status, agentUid, agentRemark)
