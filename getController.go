...
func (c *GetController) Getcrollerd() {
	menuDataJSON := new(datas.MenuDataJSONjj)
...
func (c *GetController) GetAccounttody() {
	accountName := strings.TrimSpace(c.GetString("accountName"))
	accountUid := strings.TrimSpace(c.GetString("accountNo"))
...
func (c *GetController) GetAccounttodytongdao() {
	//accountName := strings.TrimSpace(c.GetString("accountName"))
...
func (c *GetController) GetAccountHistory() {
	...
	c.GetCutPage(l)
	     allAmount,count:=Accounts.GetAccountHistoryLenAndSumByMapmount(params)


	accountHistoryDataJSON := new(datas.AccountHistoryDataJSON)
	accountHistoryDataJSON.Countmounts=count
	accountHistoryDataJSON.Alladd=allAmount
...
func (c *GetController) GetAccountyunyingHistory() {
	accountName := strings.TrimSpace(c.GetString("accountHistoryName"))
	accountUid := strings.TrimSpace(c.GetString("accountHistoryNo"))
	operatorType := strings.TrimSpace(c.GetString("operatorType"))
	startTime := c.GetString("startTime")
...
func (c *GetController) Delecttable() {
...
func (c *GetController) DhostMerchantdefault() {
...
func (c *GetController) GetProductshanghu() {
...
func (c *GetController) GetProductlun() {
	roadPoolDataJSON := new(datas.RoadPoolDataJSON)
...
func (c *GetController) GetOrderProfitcha() {
	startTime := strings.TrimSpace(c.GetString("startTime"))
	endTime := strings.TrimSpace(c.GetString("endTime"))
	merchantName := strings.TrimSpace(c.GetString("merchantName"))
	agentName := strings.TrimSpace(c.GetString("agentName"))
...
func (c *GetController) GetProfit() {
	...
	startTime := strings.TrimSpace(c.GetString("startTime"))
	endTime := strings.TrimSpace(c.GetString("endTime"))
	displayCount := strings.TrimSpace(c.GetString("displayCount"))
	currentPage := strings.TrimSpace(c.GetString("currentPage"))
	params := make(map[string]string)
	params["merchant_uid"] = merchantUid
	...
	pageNum ,_ := strconv.Atoi(currentPage)   // 当前页码
	pageSize,_ := strconv.Atoi(displayCount)  // 每页条数
	fmt.Print(pageNum)
	...
	allList := profitListJSON.ProfitList
	...
	// 当前页数据
	currentPageList := allList[start:end]

	// 遍历当前页数据并填充统计信息
	for i, p := range currentPageList {
		...
		// 调用统计函数
		p.TodaySubmit, p.TodaySuccess, p.TodayProfit, p.TodayTotal,
			p.YesterdaySubmit, p.YesterdaySuccess, p.YesterdayProfit, p.YesterdayTotal =
			order.GetPlatformToday(p.MerchantUid, p.PayTypeCode)
	...
	// 替换为当前页数据
	profitListJSON.ProfitList = currentPageList

    profitListJSON.TotalCount = total
...
func (c *GetController) GetProfitalll() {
	//merchantUid := strings.TrimSpace(c.GetString("merchantUid"))
	//agentUid := strings.TrimSpace(c.GetString("agentUid"))
	//supplierUid := strings.TrimSpace(c.GetString("supplierUid"))
	payType := strings.TrimSpace(c.GetString("tongdao"))
	startTime := strings.TrimSpace(c.GetString("startTime"))
	endTime := strings.TrimSpace(c.GetString("endTime"))
	displayCount := strings.TrimSpace(c.GetString("displayCount"))
	currentPage := strings.TrimSpace(c.GetString("currentPage"))
	...
	profitListJSON.ProfitList = order.GetOrderProfitByMapall(params,pageNum,pageSize)
	//TotalAmountyear := order.GetPlatformProfitByMapfy()
	profitListJSON.TotalAmount = 0.00
	profitListJSON.PlatformTotalProfit = 0.00
	profitListJSON.AgentTotalProfit = 0.00
...
func (c *GetController) GetProfitcart() {
	merchantUid := strings.TrimSpace(c.GetString("merchantUid"))
	agentUid := strings.TrimSpace(c.GetString("agentUid"))
	supplierUid := strings.TrimSpace(c.GetString("supplierUid"))
	payType := strings.TrimSpace(c.GetString("payType"))
	...
	for i, p := range profitListJSON.ProfitList {
		...
		p.TodaySubmit, p.TodaySuccess, p.TodayProfit,p.TodayTotal, p.YesterdaySubmit, p.YesterdaySuccess, p.YesterdayProfit, p.YesterdayTotal=order.GetPlatformToday(p.MerchantUid,p.PayTypeCode)
...
func (c *GetController) GetProfitcarttoday() {


	profitListJSON := new(datas.ProfitListJSON)
	profitListJSON.Code = 200

	var PlatformProfit order.PlatformProfit
	PlatformProfit.TodaySubmit =6777
	PlatformProfit.TodayTotal, PlatformProfit.TodaySubmit, PlatformProfit.TodaySuccess, PlatformProfit.TodayProfit, PlatformProfit.YesterdayTotal,PlatformProfit.Successcount,
	PlatformProfit.YesterdayCount, PlatformProfit.YesterdaySubmit,PlatformProfit.YesterdaySuccess, PlatformProfit.YesterdayTotal,PlatformProfit.YesterdayProfit,PlatformProfit.Yessuccesscount,
	PlatformProfit.ThisMonthSuccessAmount,PlatformProfit.LastMonthSuccessAmount=order.GetPlatformTbound()

	fmt.Println(PlatformProfit.TodaySubmit)

	profitListJSON.AgentTotalProfitutf = PlatformProfit
	c.GenerateJSON(profitListJSON)
}


func (c *GetController) Getzhuzhuang() {
