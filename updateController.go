...
*更新商户密码
 */
func (c *UpdateController) UpdatematchPassword() {
	oldPassword := c.GetString("oldPassword")
	newPassword := c.GetString("newPassword")
	twicePassword := c.GetString("twicePassword")
...
*后台更新商户密码
 */
func (c *UpdateController) UpdatematchupdatePassword() {
	newPassword := c.GetString("newPassword")
	twicePassword := c.GetString("newVertifyPassword")

	userID:= c.GetString("agentUid")
	...
	if  userID == "" {
		...
		userInfo := merchant.GetMerchantByUid(userID)
		valid := validation.Validation{}
	      if v := valid.Min(len(newPassword), 8, ".new-error"); !v.Ok {
			dataJSON.Key = v.Error.Key
			dataJSON.Msg = "新密码长度必须大于等于8个字符!"
		} else if v := valid.Max(len(newPassword), 16, ".new-error"); !v.Ok {
			dataJSON.Key = v.Error.Key
			...
			dataJSON.Msg = "密码修改成功!"
			//更新数据库的密码
			userInfo.LoginPassword = utils.GetMD5Upper(newPassword)
			merchant.UpdateMerchant(userInfo)
		}
	}
	c.GenerateJSON(dataJSON)
}
...
*后台运营改密
 */
func (c *UpdateController) UpdateyunmatchupdatePassword() {
	newPassword := c.GetString("newPassword")
	twicePassword := c.GetString("newVertifyPassword")
	...
	if  userID == "" {
		...
		userInfo := mauser.GetAccountByUidma(userID)
		valid := validation.Validation{}
		if v := valid.Min(len(newPassword), 8, ".new-error"); !v.Ok {
			dataJSON.Key = v.Error.Key
			dataJSON.Msg = "新密码长度必须大于等于8个字符!"
			...
			dataJSON.Msg = "密码修改成功!"
			//更新数据库的密码
			userInfo.AgentPassword = utils.GetMD5Upper(newPassword)
			mauser.UpdateAccountma(userInfo)
		}
	}
	c.GenerateJSON(dataJSON)
}
...
func (c *UpdateController) EditOperatorgoogle() {
	userId := strings.TrimSpace(c.GetString("userId"))
	password := strings.TrimSpace(c.GetString("password"))
	se := new(service.UpdateService)
	keyDataJSON := se.EditOperatorgoogle(userId,password)
...
func (c *UpdateController) UpdateRoadStatuspiliang() {
	roadUid := strings.TrimSpace(c.GetString("roadUid"))

	var dataJSON *datas.BaseDataJSON

	for _,k:=range strings.Split(roadUid,",") {
		se := new(service.UpdateService)
		dataJSON = se.UpdateRoadStatus(k)
...
func (c *UpdateController) OperatorAccount() {
	...
	remark="管理员:"+userID+"操作:"+remark

	se := new(service.UpdateService)
	accountDataJSON := se.OperatorAccount(accountOperator, amount, accountUid,remark,orderid)

	c.GenerateJSON(accountDataJSON)
}

func (c *UpdateController) OperatoryunAccount() {
	accountUid := strings.TrimSpace(c.GetString("accountUid"))
	accountOperator := strings.TrimSpace(c.GetString("accountOperator"))
	amount := strings.TrimSpace(c.GetString("amount"))

	se := new(service.UpdateService)
	accountDataJSON := se.OperatoryunAccount(accountOperator, amount, accountUid)
...
func (c *UpdateController) ResetAgentPasswordmatch() {
	agentUid := strings.TrimSpace(c.GetString("agentUid"))
